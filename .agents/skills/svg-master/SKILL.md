...
- **Header Y-Coordinate Rule (Critical):** NEVER use raw Y coordinates near 0 for elements inside a clip group whose clip rect starts at `y=20`. Any element with `y < 20` will paint on top of the phone bezel frame, causing the "bleeding outside rounded corners" bug. Always use translate groups: Status Bar → `translate(20, 20)`, App Bar (back + title + menu) → `translate(20, 76)`, first body content → `y ≥ 120`. This applies even when the header appears visually "simple" — always anchor it via a translate, never raw coordinates.
- **Namespaced IDs (Critical):** Every `<clipPath>`, `<filter>`, `<linearGradient>`, `<mask>`, and `<symbol>` id MUST be prefixed with a unique screen+version identifier (e.g. `v2w-`, `v1wl-`, `v2h-`). Generic IDs like `p1-clip`, `card-shadow`, `avatar-clip` will collide when multiple SVGs are injected into the same HTML document, causing the browser to resolve clips to the wrong geometry — images render with broken clips that appear to bleed outside the phone bezel. NEVER use un-prefixed IDs in any SVG that will be part of a shared canvas.
- Bottom safe area: reserve 24–34px if the screen has a fixed bottom action bar
- **Hardware Clipping & Bezels:** Mobile UI sticky elements pinned to the bottom of the screen (e.g. Navigation Bars, CTA Footers, Bottom Sheets) AND full-screen overlays (like a Lightbox or Dimmer) MUST be wrapped perfectly inside the `<g clip-path="url(#screen-clip)">` tag so their corners inherit the phone's `40px` border radius. If placed outside the closing `</g>` of the clip-path, they will render with perfectly square edges that brutally bleed over the phone's curved bezels, destroying the native illusion. Furthermore, ANY element pinned to the bottom of the screen MUST manually include the iOS Home Indicator (e.g., `<rect width="120" height="5" rx="2.5" fill="#1A1A18" />` centered at the bottom edge) to maintain platform authenticity.
- **Sticky Header Z-Index Hierarchy:** If a screen has a scrolling body (where `frontend-engineer` will attach a scroll listener), the Status Bar (notch, time, battery) and the Top App Bar (back button, title) MUST be drawn mathematically *AFTER* the `<g id="scrollable-content">` block in the SVG document tree. If you place them before the scrollable content, the user's photos and text will scroll UP and overwrite the physical notch and back button! To prevent this, wrap the headers in a `<g id="sticky-header">` placed at the very end of the document (just before sticky footers or overlays), and ALWAYS include a solid background rect (e.g. `<rect x="20" y="20" width="375" height="100" fill="#FFFFFF" />`) so the scrolling content cleanly disappears behind it.
- **System Chrome Consistency:** You must NEVER draw custom or hallucinatory Status Bars (time, battery, notch) or Bottom Navigation Bars for new screens. You MUST copy the EXACT SVG blocks for the `<!-- Status Bar -->` (which includes the standard notch and 14:47 time) and `<!-- Bottom Tab Bar -->` from the existing project screens to maintain perfect pixel-for-pixel consistency across the entire app. Do NOT mimic the system chrome from the user's uploaded reference screenshot if it contradicts the project's established chrome. Every screen in a project must have the exact same hardware illusion.
...
- **Localized SVG Clipping (Avatars & Thumbnails):** NEVER define `<clipPath>` elements for avatars globally in the `<defs>` section with specific coordinates (e.g. `<circle cx="56" cy="76">`), and NEVER apply negative transforms to `<image>` tags to align them with global clips. A standard `clipPath` operates in the transformed coordinate space of the element referencing it. If the image is translated, the clip is translated too, causing catastrophic alignment failures and bleeding images. ALWAYS encapsulate the `<clipPath>` and its target `<image>` together inside a `<g transform="...">` container. Ensure the clip's coordinates (`cx, cy, r` or `x, y, width, height`) precisely match the `x, y, width, height` of the `<image>` tag they are clipping (usually `0,0`).
